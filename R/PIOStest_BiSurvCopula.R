...
PIOStest_BiSurvCopula <- function(x1,x2,d1,d2, copula.fam, control=list(yes.exact=FALSE, yes.boot=TRUE, nboot=500, seed1=1234, same.cen=FALSE)){
  ...
  out = PIOS.fun(u1=update.out$u1,u2=update.out$u2,d1,d2,copula.fam,yes.exact=yes.exact)
  ...
  if (yes.boot) {
    ...
    if (yes.exact){
      boot_out = sapply(1:nboot,function(b){
        ...
        out_sp = PIOS.fun(u1=update.b$u1,u2=update.b$u2,d1=d1_b,d2=d2_b,copula.fam,yes.exact=yes.exact)
        out_sp$PIOS
      })
    } else{
    ...
    pval = c("onesided" = mean(boot.u>(PIOS-1)), "twosided" = mean(boot.u>abs(PIOS-1))*2)
  } else {boot_out = NA;pval=NA}
  return(list("theta_est"=theta_est,"PIOS"=PIOS,"PIOS.boot"=boot_out,"pval"=pval))
}
